...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO tbluseraccount (userid, username, password, firstname, lastname) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("issss", $userid, $username, $password, $firstname, $lastname);

    // Execute the SQL statement
    if ($stmt->execute()) {
        header("Location: login.php"); // Redirect to dashboard.php after successful registration
        exit();
    } else {
        ...
        <header>
            ...
            <div class="container">
                <div style="height: 953.094px;max-height: 100%;">
                    <h1 class="text-center" style="font-family: 'Bebas Neue', serif;color: var(--bs-body-bg);font-size: 58px;">Registration Form</h1>
                    <div style="height: 55px;"></div>
                    <div class="row register-form">
                        <div class="col-md-8 offset-md-2">
                            <form action = "registration.php" method = "POST" style="border-radius: 31px;background: rgb(45,31,84);max-width: 100%;">
                                <div class="row form-group">
                                    <div><label  style="color: var(--bs-border-color);font-family: 'Bebas Neue', serif;font-size: 21px;">username:</label></div>
                                    <div><input  type="text" name = "username" style="background: #45377b;border-radius: 9px;color: rgb(255,255,255);"></div>
                                </div>
                                <div class="row form-group">
                                    <div><label  style="font-family: 'Bebas Neue', serif;font-size: 21px;color: var(--bs-border-color);">first name:</label></div>
                                ...
                                <div class="row form-group">
                                    <div><label  style="font-family: 'Bebas Neue', serif;font-size: 21px;color: var(--bs-border-color);">password:</label></div>
                                    <div><input  type="password" name = "password" style="background: #45377b;border-radius: 9px;color: rgb(255,255,255);"></div>
                                </div>
                                <br><button class="btn btn-primary" role="button"
                                            style="background: rgb(80, 59, 140); border-color: rgba(255, 255, 255, 0);">Register</button>
                            </form>
                        </div>
                    </div>
                </div>
